...
# 인식된 색상과 DB의 색상 간의 유사도 점수를 계산
    ...
    # 각 인식된 색상에 대해 DB 색상 중 가장 유사한 것을 추출
...
#  텍스트 검색어를 받아 DB에서 '포함하는' 모든 알약 리스트를 반환
    ...
    def parse_search_terms(text):
        if not text: return []
        # 쉼표로 나누고, 앞뒤 공백 제거, 대문자 변환
        return [t.strip().upper() for t in text.split(',') if t.strip()]
    
    # 검색어 정규화 (루프 밖에서 한 번만)
    search_name = parse_search_terms(name)
    search_shape = parse_search_terms(shape)
    search_color = parse_search_terms(color)
    search_imprint = parse_search_terms(imprint)
    search_form = parse_search_terms(form)
    search_company = parse_search_terms(company)
    ...
    for row in pill_db:
        ...
        if is_match and search_shape and not any(term in db_shape for term in search_shape):
            is_match = False

        if is_match and search_color and not any(term in db_color for term in search_color):
            is_match = False
        ...
        if is_match and search_form and not any(term in db_form for term in search_form):
            is_match = False
