...
app.config["PERMANENT_SESSION_LIFETIME"] = timedelta(days=1) # 하루 동안 세션 유지
app.config["SESSION_REFRESH_EACH_REQUEST"] = True # 서버에 요청할 때 마다 세션 만료기간 갱신
...
pill_db = load_database(DB_PATH) # 데이터베이스 불러오기
if pill_db is None:
    print(f"경고: '{DB_PATH}'를 찾을 수 없습니다.")
...
def image_proxy():
    ...
    'Content-Disposition' 헤더를 제거하고 이미지로 스트리밍.
    """
    image_url = request.args.get('url')
    if not image_url:
        return "URL이 필요합니다.", 400 # 에러
    ...
    try:
        ...
        return send_file(
            ...
            as_attachment=False # "다운로드"가 아닌 "표시"하도록 함
        )
    except Exception as e:
        print(f"이미지 프록시 실패: {e}")
        return "이미지를 찾을 수 없습니다.", 404 # 에러
...
def predict():
    ...
    if 'file' not in request.files:
        return jsonify({'error': '파일이 없습니다.'}), 400 # 에러

    file = request.files.get('file')
    if not file or file.filename == '':
        return jsonify({'error': '파일이 선택되지 않았습니다.'}), 400 # 에러
...
def get_result(task_id):
    ...
    if task_result.successful():
        ...
        return jsonify({'error': '작업 실패', 'details': str(task_result.info)}), 500 # 에러
    else:
...
def detail():
    ...
    if not item_code:
        return jsonify({'error': '품목기준코드가 필요합니다.'}), 400 # 에러
        
    # 외부 API를 호출하여 결과를 가져옴.
...
def search():
    ...
    try:
        ...
        limit = int(request.args.get('limit', 10)) # 한 페이지에 10개씩
    except ValueError:
        page = 1
        limit = 10
    ...
    # DB 로드 상태 확인
    if not pill_db:
         return jsonify({'error': '서버 DB가 로드되지 않았거나 비어있습니다.'}), 500 # 에러

    # database_handler로 텍스트 검색
...
def get_recent():
    ...
    """
    if not redis_client:
        return jsonify({'error': 'Redis에 연결되지 않았습니다.'}), 500 # 에러

    # 세션 ID가 없으면 빈 리스트 반환
    if not session.sid:
        return jsonify({'pill_results': []})
    ...
    try:
        ...
        return jsonify({'error': '최근 검색 목록을 불러오는 중 오류 발생'}), 500 # 에러
...
def refresh_recent_list_expiration(response):
    """
    모든 요청 처리 후 응답을 보내기 전에 실행.
    세션이 있고, 최근 내역(db=2) 데이터가 존재하면 만료 시간을 갱신.
    """
    try:
